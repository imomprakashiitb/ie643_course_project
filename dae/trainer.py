...
class Trainer:
    def __init__(self, model, train_dataloader, val_dataloader,
        ...
        self.writer = None
        self.lr_scheduler = None  # Added to store the scheduler
        self.reset_state()
    ...
    def set_up_tensorboard(self):
        ...
        logdir = Path("/content/drive/MyDrive/ie643_course_project_24M1644/runs") / (prefix + dt_string)
    ...
    def train_epoch(self):
        ...
        for batch in self.train_dataloader:
            ...
            if self.state["train_it"] % self.state["epoch_len"] == 0:
                ...
                if self.lr_scheduler is not None:  # Step the scheduler after each epoch
    ...
    def get_saveable_state(self):
        state = {
            "epoch_no": self.state["epoch_no"],
            "train_it": self.state["train_it"],
            "model_state_dict": self.model.state_dict(),
            "model_class": self.model.__class__.__name__,
            "optimiser_state_dict": self.optimiser.state_dict()
        }
        if self.lr_scheduler is not None:  # Include scheduler state if it exists
    ...
    def save(self, path=None, name=None, **kwargs):
        ...
        path = Path("/content/drive/MyDrive/ie643_course_project_24M1644/saved_models") / f"{name}.pt" if path is None else path
    ...
    def load(self, identifier=None, load_optimiser=True, exclude_keys=[], dir="saved_models"):
        ...
        path = Path("/content/drive/MyDrive/ie643_course_project_24M1644/saved_models") / f"{identifier}.pt"
        if not path.exists():
        ...
        if self.lr_scheduler is not None and "lr_scheduler_state_dict" in checkpoint:  # Load scheduler state
